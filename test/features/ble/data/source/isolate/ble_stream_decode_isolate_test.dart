...
import 'package:vulcan_mobile_playground/features/ble/data/source/isolate/decode_worker.dart';

void main() {
  final batchWait =
      BleStreamFrameConfig.defaultBatchInterval +
  ...
  test(
    'BleStreamDecodeIsolate decodes batched EMG bytes into one snapshot',
    () async {
      final isolate = await StreamDecodeWorker.create();
      ...
      final subscription = isolate
          .decodeStream(source: controller.stream, deviceId: 'device-1')
          .listen((snapshot) {
  ...
  test(
    'BleStreamDecodeIsolate batches multiple frames into one snapshot',
    () async {
      final isolate = await StreamDecodeWorker.create();
      addTearDown(isolate.dispose);
      ...
      expect(
        snapshots.first.rawBytes,
        hasLength(BleStreamFrameConfig.emgFrameSizeBytes * 3),
      );
    },
  );

  test(
    'StreamDecodeWorker drops oldest pending batch while decode is in flight',
    () async {
      final isolate = await StreamDecodeWorker.create();
      addTearDown(isolate.dispose);
